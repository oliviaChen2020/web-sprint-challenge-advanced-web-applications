...
1. Explain what a token is used for.
   Token- based authentication is a protocol which allows users to verify their identity and in return receive a unique access token, Auth-token works like a ticket for a identified user to retain access as long as the token remain valid

2. What steps can you take in your web apps to keep your data secure?
   Clients needs to provide login credentials for the server to issue an authentication token, then the server makes decision for data transfer based on client’s known credentials.

3. Describe how web servers work.
   Web servers is connected to the internet and it stores the code for different websites to be shared across the world at all times. when we type a URL into the web browser, you are requesting that page from a server then the web server sends the page back.
